...
from heart_stroke.entity.config_entity import *
from heart_stroke.entity.raw_data_validation import IngestedDataValidation
from heart_stroke.entity.artifact_entity import DataValidationArtifact, DataIngestionArtifact
    ...
    def is_Validation_successfull(self):
        try:
            ...
            if self.isFolderPathAvailable() == True:
                ...
                logging.info(
                    f"Train_set status'' {train_filename}''''''{is_train_filename_validated}''''''{is_train_column_numbers_validated}''''{is_train_column_name_same}''''{is_train_missing_values_whole_column}")
                logging.info(
                    f"Test_set status'' {test_filename}''''''{is_test_filename_validated}''''''{is_test_column_numbers_validated}''''{is_test_column_name_same}''''{is_test_missing_values_whole_column}")
